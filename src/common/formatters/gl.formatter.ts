...
function buildGLMainOrder(rawNames: string[], fuelNames: string[]) {
  const order: string[] = [];
...
// ================= 排序整个方案 =================
export function sortGLResult(scheme: any) {
  // 提取原料和燃料的矿耗数据
  ...
  // 构建主要参数排序的顺序
  const mainParamOrder = buildGLMainOrder(rawNames, fuelNames);
...
// ================= 完整格式化 =================
export function formatGLResultFull(
  result: any,
  ...
  if (result["主要参数"]) {
    ...
    const mainParamOrder = buildGLMainOrder(extractMaterialNames(raw), extractMaterialNames(fuel));
