...
// ================= 排序整个方案 =================
export function sortTQYTHResult(scheme: any) {
  // 提取原料和燃料的矿耗数据
  ...
  // 构建主要参数排序的顺序
  const mainParamOrder = buildTQYTHMainOrder(rawNames, fuelNames);
...
// ================= 完整格式化 =================
export function formatTQYTHResultFull(
  result: any,
  ...
  if (result["主要参数"]) {
    ...
    if (main['固定费用'] != null) newMain[mainUnitMap['固定费用']] = main['固定费用'];

    if (main['生料比影响综合焦比'] != null) newMain[mainUnitMap['生料比影响综合焦比']] = main['生料比影响综合焦比'];

    const mainParamOrder = buildTQYTHMainOrder(extractMaterialNames(raw), extractMaterialNames(fuel));
