...
export class SystemLatestSchemeService {
  constructor(
    @InjectRepository(SystemLatestScheme) private readonly repo: Repository<SystemLatestScheme>,
    @InjectRepository(Task) private readonly taskRepo: Repository<Task>,
  ) {}

  async setTodayLatest(user: User, taskUuid: string, schemeIndex: number, moduleType: ModuleTypeEnum) {
    const task = await this.taskRepo.findOne({ where: { task_uuid: taskUuid }, relations: ['results'] });
    if (!task) return ApiResponse.error('任务不存在');

    let results = task.results?.[0]?.output_data;
    if (!results) return ApiResponse.error('任务结果为空');
    if (typeof results === 'string') {
      try { results = JSON.parse(results); } catch { return ApiResponse.error('任务结果解析失败'); }
    }
    ...
    let record = await this.repo.findOne({ where: { scheme_date: today, module_type: moduleType }, relations: ['modifier', 'task'] });
    ...
    if (record) {
      ...
      record = await this.repo.save(
        this.repo.create({ scheme_date: today, module_type: moduleType, task, scheme_id: schemeId, result: scheme, modifier: user }),
      );
    }
  ...
  async getRecentLatest(days = 1, moduleType?: ModuleTypeEnum) {
    const today = dayjs().startOf('day');
    const startDay = today.subtract(days - 1, 'day');

    const where: any = { scheme_date: Between(startDay.format('YYYY-MM-DD'), today.format('YYYY-MM-DD')) };
    if (moduleType) where.module_type = moduleType;

    const records = await this.repo.find({ where, relations: ['modifier'], order: { scheme_date: 'DESC', module_type: 'ASC' } });

    if (!records?.length) return ApiResponse.success([], `最近${days}天暂无系统最新方案`);
