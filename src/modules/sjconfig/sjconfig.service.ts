...
export class SjconfigService {
  ...
  async getLatestConfigByName(user: User, moduleName: string) {
    try {
      this.logger.log(`获取最新参数组，userId=${user.user_id}, moduleName=${moduleName}`);
...
async saveIngredientParams(user: User, moduleName: string, ingredientParams: number[]) {
  try {
    this.logger.log(`保存 ingredientParams，userId=${user.user_id}, moduleName=${moduleName}, params=${JSON.stringify(ingredientParams)}`);
