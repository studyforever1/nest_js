...
async query(params: PortPelletLumpPaginationDto) {
  ...
  async importExcelBatchUpdate(file: Express.Multer.File, username: string) {
    ...
    const rowPayloads: Array<{ name: string; port?: string; updates: Record<string, number> }> = [];
    ...
    sheet.eachRow({ includeEmpty: true }, (row, index) => {
      ...
      const portText = headerMap['港口'] ? this.cellToString(row.getCell(headerMap['港口']).value) : undefined;
      const updates: Record<string, number> = {};

      FIXED_HEADERS.forEach((key) => {
        if (key === keyHeader || key === '港口') return;
        const col = headerMap[key];
        if (!col) return;
      ...
      rowPayloads.push({ name, port: portText ? portText : undefined, updates });
    });
    ...
    const updates: PortPelletLumpInfo[] = [];
    let skipped = 0;
    ...
    rowPayloads.forEach((row) => {
      ...
      const hasAnyField = !!row.port || Object.keys(row.updates).length > 0;
      if (!hasAnyField) {
        skippedNoChange += 1;
        return;
      }

      if (row.port) target.port = row.port;
  ...
  private readonly batchUpdateTemplateFilename = 'port-pellet-lump-batch-update-template.xlsx';
