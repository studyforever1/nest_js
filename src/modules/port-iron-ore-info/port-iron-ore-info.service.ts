...
async query(params: PortIronOrePaginationDto) {
  ...
  async importExcelBatchUpdate(file: Express.Multer.File, username: string) {
    ...
    sheet.eachRow({ includeEmpty: true }, (row, index) => {
      ...
      const port = headerMap['港口'] ? this.cellToString(row.getCell(headerMap['港口']).value) : undefined;
      ...
      rowPayloads.push({ name, port: port ? port : undefined, updates });
    });
  ...
  private readonly batchUpdateTemplateFilename = 'port-iron-ore-info-batch-update-template.xlsx';
