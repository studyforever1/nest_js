...
export class LlythCalcService {
  ...
  async startTask(moduleName: string, user: User): Promise<ApiResponse<{ taskUuid: string; resultMap: Record<string, any> }>> {
    try {
      ...
      // ---------- 燃料处理 ----------
      const fuelIds = config.fuelParams || [];
      ...
      fuelIds.forEach(id => {
        ...
        if (fuel && limit) fuelLimits[String(id)] = { low_limit: safeNumber(limit.low_limit), top_limit: safeNumber(limit.top_limit) };
      });
      ...
      const fullParams = {
        ...
        fuelParams,
        fuelLimits,
        slagLimits: Object.fromEntries(Object.entries(config.slagLimits || {}).map(([k, v]: any) => [k, { low_limit: safeNumber(v.low_limit), top_limit: safeNumber(v.top_limit) }])),
...
async fetchAndSaveProgress(taskUuid: string, pagination?: PaginationDto): Promise<ApiResponse<any>> {
  try {
    ...
    if (task.status !== TaskStatus.FINISHED) {
      ...
      incoming.forEach(item => {
        Object.keys(item["原料配比和矿耗"] || {}).forEach(idStr => idSet.add(Number(idStr)));
        Object.keys(item["燃料配比和矿耗"] || {}).forEach(idStr => idSet.add(Number(idStr)));
      });
      ...
      // 过滤出有“本月毛利”的方案
      const validResults = incoming.filter(item => item["主要参数"] && typeof item["主要参数"].本月毛利 === "number");

      // 1️⃣ 利润排名（按本月毛利降序）
      validResults.sort((a, b) => b["主要参数"].本月毛利 - a["主要参数"].本月毛利);
      validResults.forEach((item, index) => item.利润排名 = index + 1);
  ...
  async getSchemeByIndex(taskUuid: string, schemeIndex: number): Promise<ApiResponse<any>> {
    ...
    if (Array.isArray(resultEntity.output_data)) allResults = resultEntity.output_data;
      ...
      try { allResults = JSON.parse(resultEntity.output_data); } catch (err) { return ApiResponse.error('结果解析错误'); }
    }
    ...
    const processMaterials = async (field: string, limitsMap: Record<string, any>) => {
      ...
      Object.entries(data).forEach(([id, val]) => {
        ...
        result[id] = { ...val, name: val.name || idNameMap[id] || id, low_limit: limits.low_limit ?? 0, top_limit: limits.top_limit ?? 100 };
      });
      return result;
    };
    ...
    const processValuesWithLimits = (data: Record<string, any>, limitsMap: Record<string, any>, lowDefault = 0, topDefault = 100) => {
      ...
      Object.entries(data || {}).forEach(([key, val]) => {
        ...
        result[key] = { value: val, low_limit: limits.low_limit ?? lowDefault, top_limit: limits.top_limit ?? topDefault };
      });
      return result;
    };
