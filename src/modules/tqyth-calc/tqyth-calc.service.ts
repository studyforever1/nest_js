...
import { formatTQYTHResultFull, sortTQYTHResult } from 'src/common/formatters/tqyth.formatter';
...
export class TqythCalcService {
  ...
  async stopTask(taskUuid: string): Promise<ApiResponse<{ taskUuid: string; status: string }>> {
    try {
      ...
      // 1️⃣ 通知 FastAPI 停止
      await this.apiPost('/tqyth/stop/', { taskUuid });
  ...
  async fetchAndSaveProgress(
    ...
    try {
      ...
      if (FINAL_STATUS.includes(task.status) && !forceSave) {
        ...
        results = results.map(item => sortTQYTHResult(item));
      }
      // ================== RUNNING状态处理 ==================
      else {
        ...
        if (needFormat) {
          ...
          results.forEach(item => sortTQYTHResult(item));
